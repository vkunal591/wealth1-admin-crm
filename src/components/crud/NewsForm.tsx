...
import { NewsFormType } from "./formInput/NewsFormType";
...
const NewsForm: React.FC<NewsProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormFields] = useState<any>(
    data?._id ? populateFormFields(NewsFormType, data) : NewsFormType
  );
  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(NewsFormType, data) : {}
  );
