...
import { ProductsFormType } from "./formInput/ProductsFormType";
...
const ProductsForm: React.FC<ProductsProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormFields] = useState<any>(
    data?._id ? populateFormFields(ProductsFormType, data) : ProductsFormType
  );
  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(ProductsFormType, data) : {}
  );
